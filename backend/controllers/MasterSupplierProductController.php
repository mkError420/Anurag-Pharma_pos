...
class MasterSupplierProductController {
    ...
    public static function create($requestData) {
        ...
        try {
            ...
            $checkStmt = $pdo->prepare("SELECT id, supplier_name, product_name FROM master_supplier_products WHERE LOWER(supplier_name) = LOWER(?) AND LOWER(product_name) = LOWER(?)");
        ...
        try {
            ...
            foreach ($items as $idx => $item) {
                ...
                if ($insertStmt->rowCount() > 0) {
                    ...
                    $skippedCount++; // Duplicate or not inserted
                }
            }
            ...
            header('Content-Type: application/json');
            echo json_encode([
                'message' => "Bulk upload complete. {$insertedCount} inserted, {$skippedCount} skipped/duplicates.",
