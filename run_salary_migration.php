<?php
// Run migration to add Staff Salary columns and staff_salaries table
require_once 'backend/config/db.php';

try {
    echo "Starting migration to add Staff Salary settings and staff_salaries table...\n";
    $pdo = DB::getConnection();

    // Check if columns exist in users table
    $stmt = $pdo->query("SHOW COLUMNS FROM `users` LIKE 'base_salary'");
    if (!$stmt->fetch()) {
        echo "Adding salary columns to `users` table...\n";
        ...
        echo "Salary columns already exist in `users` table.\n";
    }

    // Create staff_salaries table if not exists
    echo "Creating `staff_salaries` table...\n";
    ...
    echo "`staff_salaries` table created/verified successfully.\n";
    echo "Migration completed successfully!\n";

} catch (Exception $e) {
    echo "Error during salary migration: " . $e->getMessage() . "\n";
    exit(1);
}
