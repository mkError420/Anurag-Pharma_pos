...
export default function SalesHistory() {
  ...
  const currentWindowIndex = Math.floor((currentPage - 1) / maxVisiblePages);
  const startPage = currentWindowIndex * maxVisiblePages + 1;
      ...
      {totalPages > 1 && (
        <div className="bg-white border border-slate-200 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-xs">
          <div className="text-xs font-semibold text-slate-500">
            Showing <span className="text-slate-800">{filteredSales.length > 0 ? indexOfFirstSale + 1 : 0}</span> to <span className="text-slate-800">{Math.min(indexOfLastSale, filteredSales.length)}</span> of <span className="text-slate-800">{filteredSales.length}</span> entries
          </div>
          <div className="flex items-center flex-wrap gap-1.5 justify-center sm:justify-end">
            {/* Quick jump to previous 20 pages */}
            {startPage > 1 && (
              <button
                onClick={() => setCurrentPage(startPage - 1)}
                title="Previous 20 pages"
                className="w-9 h-9 rounded-xl text-xs font-bold bg-white hover:bg-slate-50 text-slate-600 border border-slate-200 transition-all flex items-center justify-center shadow-xs"
              >
                «
              </button>
            )}
            ...
            <button
              ...
              className="px-3 py-2 bg-white hover:bg-slate-50 disabled:hover:bg-white disabled:opacity-50 text-slate-600 border border-slate-200 rounded-xl text-xs font-semibold transition-colors disabled:cursor-not-allowed flex items-center gap-1"
            >
                  ...
                  <>
                    ...
                    <div className="mt-5 flex justify-end space-x-3">
                      <button
                        ...
                        className="bg-sky-500 hover:bg-sky-600 text-white font-bold py-2.5 px-6 rounded-xl text-sm transition-colors flex items-center space-x-2"
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
