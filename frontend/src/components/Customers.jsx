...
export default function Customers() {
  ...
  const promptSingleDelete = (customer) => {
  ...
  const handleConfirmDelete = async () => {
    const ids = customerToDelete ? [customerToDelete.id] : [...selectedCustomerIds];
    ...
    setDeleteProgress({
      ...
      total: totalItems,
      percent: 0,
      currentName: customerToDelete ? customerToDelete.name : `Starting deletion of ${totalItems} customer(s)...`
    });
    ...
    try {
      for (let i = 0; i < ids.length; i += chunkSize) {
        ...
        const firstCust = customers.find(c => c.id === chunk[0]);
        setDeleteProgress({
          active: true,
          current: i,
          total: totalItems,
          percent: Math.round((i / totalItems) * 100),
          currentName: firstCust ? firstCust.name : `Deleting batch (${i + 1}-${Math.min(i + chunkSize, totalItems)})...`
        });
        ...
        setDeleteProgress({
          ...
          currentName: `Processed ${processed} of ${totalItems} customers`
        });
      }
      ...
      if (failureCount > 0) {
        triggerAlert(
          'warning',
          `Deleted ${successCount} customer(s). ${failureCount} customer(s) could not be deleted because they are referenced in transaction records.`
        );
      } else {
        triggerAlert('success', `Successfully deleted ${successCount || totalItems} customer(s)!`);
      }
      ...
      triggerAlert('error', err.message || 'Failed to delete customers.');
    } finally {
      setDeleting(false);
    }
  };
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
          <button
            ...
            className="bg-amber-100 hover:bg-amber-200 text-amber-900 font-semibold py-2.5 px-4 rounded-xl text-sm shadow transition-colors flex items-center space-x-2"
          >
          ...
          <button
            ...
            className="bg-sky-500 hover:bg-sky-600 text-white font-semibold py-2.5 px-4 rounded-xl text-sm shadow transition-colors flex items-center space-x-2"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-xs flex items-center space-x-3">
          ...
          <div>
            ...
            <h3 className="text-xl font-extrabold text-slate-800">{customers.length}</h3>
          </div>
        </div>
        {/* You can add more summary cards here for things like total due balance, etc. */}
      </div>
            ...
            <thead>
              <tr className="border-b border-slate-100 text-xs font-bold text-slate-400 uppercase tracking-wider bg-slate-50/50">
                <th className="p-4 w-12">
                  <input
                    type="checkbox"
                    checked={currentCustomers.length > 0 && currentCustomers.every(c => selectedCustomerIds.includes(c.id))}
                    onChange={(e) => {
                      if (e.target.checked) {
                        const allIds = currentCustomers.map(c => c.id);
                        setSelectedCustomerIds(Array.from(new Set([...selectedCustomerIds, ...allIds])));
                      } else {
                ...
                currentCustomers.map((customer, index) => (
                  <tr key={customer.id} className={`hover:bg-slate-50/50 transition-colors ${searchFocusedIndex === index ? 'bg-indigo-100 ring-2 ring-indigo-500 ring-inset' : ''}`}>
                    ...
                    <td className="p-4 text-center space-x-2 whitespace-nowrap">
                      ...
                      <button
                        onClick={() => promptSingleDelete(customer)}
        ...
        ৳{totalCollectAmount.toFixed(2)}
                  ...
                  {currentDue > 0 && (
                    <button
                      ...
                      className="bg-emerald-100 hover:bg-emerald-200 text-emerald-800 font-semibold py-3 px-5 rounded-xl text-xs transition-colors shadow-sm flex items-center justify-center space-x-1.5 whitespace-nowrap w-full lg:w-auto shrink-0"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                          ...
                          <form onSubmit={handleSaveEditSale} className="space-y-4 bg-white p-4 rounded-xl border border-slate-200/80 shadow-inner text-xs">
                            ...
                            <div className="flex space-x-2 items-end bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                              ...
                              <button
                                ...
                                className="bg-indigo-100 hover:bg-indigo-200 text-indigo-800 font-semibold py-1.5 px-3 rounded-lg transition-colors text-xs h-[29px]"
                              >
                                Add
                              </button>
                            </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-fadeIn">
            <div className="p-6 text-center">
              ...
              <h3 className="text-lg font-bold text-slate-800 mb-2">
                {customerToDelete ? 'Delete Customer Profile' : 'Delete Selected Customers'}
              </h3>
              ...
              {!deleting ? (
                <p className="text-sm text-slate-500 mb-6">
                  {customerToDelete ? (
                    ...
                    <>
                      Are you sure you want to permanently delete <span className="font-semibold text-rose-600">{selectedCustomerIds.length}</span> selected customer(s)?
                      This action cannot be undone.
                    </>
                  )}
                </p>
              ...
              <div className="flex justify-center space-x-3 mt-4">
                ...
                >
                  {deleting ? (
                    ...
                    <span>{customerToDelete ? 'Delete' : 'Confirm Delete'}</span>
                  )}
                </button>
              </div>
            </div>
